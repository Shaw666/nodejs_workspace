...
var Session      = require("./session-ipr-1.js");

var IDLE_TIME = 300;

function Daemon_ipr_1(server, line) {
    Daemon_tcp.call(this, server, line);
    this.socket_server = null;
}
util.inherits(Daemon_ipr_1, Daemon_tcp);

Daemon_ipr_1.prototype.on_stat_report = function (session, report) {
    this.log("report: ", report);
    this.emit("stat-report", this, session, report);
};

Daemon_ipr_1.prototype.on_cid_event = function (session, event) {
    this.log("[cid event]", event.cid);
    this.emit("cid-event", this, session, event);
};

Daemon_ipr_1.prototype.on_new_connection = function (socket) {
...
Daemon_ipr_1.prototype.check_session_timeout = function () {
    ...
    for (var sid in this.sessions) {
        ...
        if (session.idle_timer_counter > t_timeout) {
            session.log("timeout, disconnect ...");
            session.close();
        }
    }
};
Daemon_ipr_1.prototype.start = function () {
